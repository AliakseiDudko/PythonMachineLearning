...
def get_data_frame_svm_classifier_score(tbl) -> (float, float):
    train_data, test_data, train_survived_data, test_survived_data = featureEngineering.split_data_frame(tbl)

    svm_classifier = sklearn.svm.SVC(kernel="poly")
    svm_classifier.fit(train_data, train_survived_data.values.ravel())
    score_train = svm_classifier.score(train_data, train_survived_data)
    score_test = svm_classifier.score(test_data, test_survived_data)
...
def get_svm_classifier_score(settings) -> (float, float):
    ...
    return get_data_frame_svm_classifier_score(tbl)
...
def find_best_svm_classifier_score() -> (float, dict):
    ...
    for settings_seed in range(0, featureEngineering.get_settings_variations_count()):
        ...
        for attempt in range(0, attempt_count):
            ...
            score_train, score_test = get_data_frame_svm_classifier_score(tbl)
        ...
        if best_test_score < score_test_avg:
            ...
            print(f"SVM best configuration: {best_test_score}, settings: {best_settings}")
